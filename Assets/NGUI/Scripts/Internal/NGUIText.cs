...
static public class NGUIText
{
	...
	static public void Update (bool request)
	{
		...
		useSymbols = hasEmojis || (bitmapFont != null && bitmapFont.hasSymbols) && encoding && symbolStyle != SymbolStyle.None;
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
				...
				{
					EmojiAlign(verts, currentLineStartIndex, verts.size, currentLineHeight);
				}
				currentLineStartIndex = verts.size;
    ...
    /// </summary>
    static public void PrintEmojis(string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
    {
		if (string.IsNullOrEmpty(text)) return;
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
				...
				if (alignment != Alignment.Left && indexOffsetEmoji < verts.size)
				{
                    if (hasEmojis)
                    {
                        Align(verts, indexOffsetEmoji, x - finalSpacingX);
                        indexOffsetEmoji = verts.size;
                    }
                }
	...
	static void EmojiAlign (BetterList<Vector3> verts, int start, int end, float maxLineHeight) {
        float offset = 0;
        switch (emojiAlignment) {
